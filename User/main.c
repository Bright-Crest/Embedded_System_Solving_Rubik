...
{		
  ...
  ILI9341_DispStringLine_EN(LINE(0),"BH OV7725 Test Demo");
	...
	INFO_PRINTF("OV7725摄像头初始化完成\r\n");
	
	Ov7725_vsync = 0;
	
	/*通过控制Ov7725_vsync来控制是否停止相机下一帧的写入相机缓存FIFO，如停止，LCD也停止*/
	int8_t is_stop_camera = 0;
	while(1)
	{
		/*接收到新图像进行显示*/
		if( Ov7725_vsync == 2 && is_stop_camera == 0)
		{
			frame_count++;
			FIFO_PREPARE;  			/*FIFO准备*/					
		...
		/*检测按键：通过控制is_stop_camera控制相机数据接收*/
		if( Key_Scan(KEY1_GPIO_PORT,KEY1_GPIO_PIN) == KEY_ON  )
		{
			/*LED反转*/
			LED_GREEN_TOGGLE;
            /*is_stop_camera反转*/
			is_stop_camera = (is_stop_camera + 1) % 2;

			DEBUG_PRINTF("key 1: stop/start the camera\n");
		} 

		/*检测按键：通过串口传输当前FIFO中的图片数据，要求已经stop camera，因为不可以让LCD和串口同时读FIFO*/
		if( Key_Scan(KEY2_GPIO_PORT,KEY2_GPIO_PIN) == KEY_ON && is_stop_camera == 1)
		{
			/*LED反转*/
			LED_BLUE_TOGGLE;	
            ...
            // write_rgb(0, cam_mode.cam_width, cam_mode.cam_height);
		    Screen_Shot2Usart(0, 0, cam_mode.cam_width, cam_mode.cam_height, "0");
			DEBUG_PRINTF("key 2: write_rgb_wincc finished\n");
#endif /*SEND_IMAGE_UPPER*/
