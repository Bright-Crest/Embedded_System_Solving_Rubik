...
{
  DEBUG_PRINTF("in receiving_process: 接收的数据处理\n");
...
{
  DEBUG_PRINTF("in set_wincc_format: 发送设置图像格式包给上位机\n");
  uint16_t crc_16 = 0xFFFF;
...
{
  DEBUG_PRINTF("in write_rgb_wincc: 发送图像数据包给上位机\n");
 ...
 * @return 0：成功，-1：失败.
 */
int write_rgb(uint8_t addr, uint16_t width, uint16_t height) 
{
  DEBUG_PRINTF("in write_rgb: 发送图像数据包给上位机\n");
